...
export class SpeechService extends Disposable implements ISpeechService {
	...
	async createSpeechToTextSession(token: CancellationToken, context: string = 'speech'): Promise<ISpeechToTextSession> {
		...
		const onSessionStoppedOrCanceled = () => {
			...
			// Complete the session before disposing
			if ('complete' in session && typeof (session as any).complete === 'function') {
				(session as any).complete();
			}
		...
		// Add the session itself to disposables for cleanup
		if ('dispose' in session && typeof session.dispose === 'function') {
			disposables.add(session as any);
		}
		...
		disposables.add(session.onDidChange(e => {
			...
			case SpeechToTextStatus.Stopped:
				// NO ACTION - disposal handled by cancellation only
				break;
