...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	constructor(
		...
		if (
			...
			ChatContextKeys.Setup.hidden.bindTo(this.contextKeyService).set(true); // hide UI
			return;
		}

		// For MedCompanion: Skip GitHub entitlement checks, mark as installed
		ChatContextKeys.Setup.installed.bindTo(this.contextKeyService).set(true);
