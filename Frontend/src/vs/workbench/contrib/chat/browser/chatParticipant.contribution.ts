...
const editsViewContainer: ViewContainer = Registry.as<IViewContainersRegistry>(ViewExtensions.ViewContainersRegistry).registerViewContainer({
	...
	title: localize2('chatEditing.viewContainer.label', "Edits"),
...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Chat failed to load because the installed version of the Chat extension is not compatible with this version of {0}. Please ensure that the Chat extension is up to date.", this.productService.nameLong);
		...
		const versionMessage = `Chat version: ${chatExtension.version}`;
...
const baseEditsViewDescriptor: IViewDescriptor = {
	...
	openCommandActionDescriptor: {
		...
		mnemonicTitle: localize({ key: 'miToggleEdits', comment: ['&& denotes a mnemonic'] }, "Ed&&its"),
		keybindings: {
...
class EditsViewContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(): void {
		const welcomeViewMainMessage = localize('editsMovedMainMessage', "Edits has been moved to the [main Chat view](command:workbench.action.chat.open). You can switch between modes by using the dropdown in the Chat input box.");
