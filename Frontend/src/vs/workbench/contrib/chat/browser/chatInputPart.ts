...
import { ChatAgentLocation, ChatConfiguration, ChatDomain, ChatMode, validateChatDomain, validateChatMode } from '../common/constants.js';
import { ILanguageModelChatMetadataAndIdentifier, ILanguageModelsService } from '../common/languageModels.js';
import { CancelAction, ChatEditingSessionSubmitAction, ChatSubmitAction, ChatSwitchToNextModelActionId, IChatExecuteActionContext, IToggleChatModeArgs, IToggleDomainArgs, ToggleAgentModeActionId, ToggleDomainActionId } from './actions/chatExecuteActions.js';
import { AttachToolsAction } from './actions/chatToolActions.js';
import { ImplicitContextAttachmentWidget } from './attachments/implicitContextAttachment.js';
import { DicomImplicitContextAttachmentWidget } from './attachments/dicomImplicitContextAttachment.js';
...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	public getAttachedAndImplicitContext(sessionId: string): IChatRequestVariableEntry[] {
		...
		// Add DICOM implicit context
		if (this.dicomImplicitContext?.enabled && this.dicomImplicitContext.value) {
			contextArr.push(this.dicomImplicitContext.toBaseEntry());
		}
	...
	private _dicomImplicitContext: DicomImplicitContext | undefined;
	public get dicomImplicitContext(): DicomImplicitContext | undefined {
		return this._dicomImplicitContext;
	}
	...
	private chatDomain: IContextKey<ChatDomain>;
	...
	private _onDidChangeCurrentChatDomain = this._register(new Emitter<ChatDomain>());
	readonly onDidChangeCurrentChatDomain = this._onDidChangeCurrentChatDomain.event;

	private _currentDomain: ChatDomain = ChatDomain.General;
	public get currentDomain(): ChatDomain {
	...
	constructor(
		...
		this.getInputState = (): IChatInputState => {
			return {
				...
				chatDomain: this._currentDomain,
			};
		};
		...
		this.chatDomain = ChatContextKeys.chatDomain.bindTo(contextKeyService);
	...
	setChatMode(mode: ChatMode): void {
		...
		mode = validateChatMode(mode) ?? (this.location === ChatAgentLocation.Panel ? ChatMode.Consult : ChatMode.Plan);
		this._currentMode = mode;
		this.chatMode.set(mode);
		this._onDidChangeCurrentChatMode.fire();
	}

	setChatDomain(domain: ChatDomain): void {
		domain = validateChatDomain(domain) ?? ChatDomain.General;
		this._currentDomain = domain;
		this.chatDomain.set(domain);
		this._onDidChangeCurrentChatDomain.fire(domain);
		this.saveState();
	}
	...
	private modelSupportedForDefaultAgent(model: ILanguageModelChatMetadataAndIdentifier): boolean {
		...
		if (this.currentMode === ChatMode.Agent || (this.currentMode === ChatMode.Plan && this.configurationService.getValue(ChatConfiguration.Edits2Enabled))) {
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		if (this.options.enableImplicitContext) {
			...
			// Initialize DICOM implicit context
			this._dicomImplicitContext = this._register(new DicomImplicitContext());
			this._register(this._dicomImplicitContext.onDidChangeValue(() => this._handleAttachedContextChange()));
		}
		...
		this.executeToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, toolbarsContainer, this.options.menus.executeToolbar, {
			...
			actionViewItemProvider: (action, options) => {
				...
				} else if (action.id === ToggleDomainActionId && action instanceof MenuItemAction) {
					const delegate: IDomainPickerDelegate = {
						getDomain: () => this.currentDomain,
						onDidChangeDomain: this._onDidChangeCurrentChatDomain.event
					};
					return this.instantiationService.createInstance(ToggleChatDomainActionViewItem, action, delegate);
				}
	...
	private renderAttachedContext() {
		...
		const hasAttachments = Boolean(attachments.length) || Boolean(this.implicitContext?.value) || Boolean(this.dicomImplicitContext?.value) || !this.instructionAttachmentsPart.empty;
		...
		if (this.dicomImplicitContext?.value) {
			const dicomPart = store.add(this.instantiationService.createInstance(DicomImplicitContextAttachmentWidget, this.dicomImplicitContext));
			container.appendChild(dicomPart.domNode);
		}
...
class ToggleChatModeActionViewItem extends DropdownMenuActionViewItemWithKeybinding {
		...
		const actionProvider: IActionProvider = {
			getActions: () => {
				const agentStateActions = [
					makeAction(ChatMode.Plan),
					makeAction(ChatMode.Diagnose),
				];
				...
				if (chatService.unifiedViewEnabled) {
					agentStateActions.unshift(makeAction(ChatMode.Consult));
				}
	...
	private modeToString(mode: ChatMode) {
		switch (mode) {
			...
			case ChatMode.Plan:
				return localize('chat.planMode', "Plan");
			case ChatMode.Diagnose:
				return localize('chat.diagnoseMode', "Diagnose");
			case ChatMode.Consult:
			default:
				return localize('chat.consultMode', "Consult");
		}
	}
...
interface IDomainPickerDelegate {
	onDidChangeDomain: Event<ChatDomain>;
	getDomain(): ChatDomain;
}

class ToggleChatDomainActionViewItem extends DropdownMenuActionViewItemWithKeybinding {
	constructor(
		action: MenuItemAction,
		private readonly delegate: IDomainPickerDelegate,
		@IContextMenuService contextMenuService: IContextMenuService,
		@IKeybindingService keybindingService: IKeybindingService,
		@IContextKeyService contextKeyService: IContextKeyService,
	) {
		const makeAction = (domain: ChatDomain): IAction => ({
			...action,
			id: domain,
			label: this.domainToString(domain),
			class: undefined,
			enabled: true,
			checked: delegate.getDomain() === domain,
			run: async () => {
				const result = await action.run({ domain } satisfies IToggleDomainArgs);
				this.renderLabel(this.element!);
				return result;
			}
		});
		...
		const actionProvider: IActionProvider = {
			getActions: () => [
				makeAction(ChatDomain.General),
				makeAction(ChatDomain.Radiology),
				makeAction(ChatDomain.Pathology),
				makeAction(ChatDomain.Dermatology),
			]
		};

		super(action, actionProvider, contextMenuService, undefined, keybindingService, contextKeyService);
		this._register(delegate.onDidChangeDomain(() => this.renderLabel(this.element!)));
	}

	private domainToString(domain: ChatDomain) {
		switch (domain) {
			case ChatDomain.General:
				return localize('chat.generalDomain', "General");
			case ChatDomain.Radiology:
				return localize('chat.radiologyDomain', "Radiology");
			case ChatDomain.Pathology:
				return localize('chat.pathologyDomain', "Pathology");
			case ChatDomain.Dermatology:
				return localize('chat.dermatologyDomain', "Dermatology");
			default:
				return localize('chat.generalDomain', "General");
		}
	}
	...
	protected override renderLabel(element: HTMLElement): IDisposable | null {
		this.setAriaLabelAttributes(element);
		const state = this.domainToString(this.delegate.getDomain());
		dom.reset(element, dom.$('span.chat-domain-label', undefined, state), ...renderLabelWithIcons(`$(chevron-down)`));
		return null;
	}

	override render(container: HTMLElement): void {
		super.render(container);
		container.classList.add('chat-domainPicker-item');
	}
}
