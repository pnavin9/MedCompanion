...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private renderSampleQuestions() {
		if (this.viewModel?.getItems().length === 0) {
			...
			this.renderFollowups(this.input.currentMode === ChatMode.Consult ? this.viewModel.model.sampleQuestions : undefined);
		}
	}
	...
	private async _acceptInput(query: { query: string } | undefined, options?: IChatAcceptInputOptions): Promise<IChatResponseModel | undefined> {
		...
		if (this.viewModel) {
			...
			if (this.viewOptions.enableWorkingSet !== undefined && this.input.currentMode !== ChatMode.Consult) {
