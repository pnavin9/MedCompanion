...
import { IChatAgentHistoryEntry, IChatAgentImplementation, IChatAgentRequest, IChatAgentResult, IChatAgentService, IChatWelcomeMessageContent } from '../common/chatAgents.js';
...
import { MedGemmaClient, IToolSchema } from './medgemma/medgemmaClient.js';
// import { WorkspaceFileScanner } from './medicalSummary/workspaceFileScanner.js';
import { MedicalSummaryDetector } from './medicalSummary/summaryDetector.js';
import { IFileService } from '../../../../platform/files/common/files.js';
import { IWorkspaceContextService } from '../../../../platform/workspace/common/workspace.js';
import { IMcpService } from '../../mcp/common/mcpTypes.js';
import { MedGemmaToolCallHandler } from './medgemma/medgemmaToolCallHandler.js';
...
class SetupChatAgentImplementation extends Disposable implements IChatAgentImplementation {
	...
	static register(instantiationService: IInstantiationService, location: ChatAgentLocation, isToolsAgent: boolean, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { disposable: IDisposable; agent: SetupChatAgentImplementation } {
		return instantiationService.invokeFunction(accessor => {
			...
			let id: string;
			let description = localize('chatDescription', "What can I assist you with today?");
			let welcomeMessageContent: IChatWelcomeMessageContent | undefined;
			const baseMessage = localize('chatMessage', "Assistant is powered by AI, so mistakes are possible. Review output carefully before use.");
			switch (location) {
				...
				case ChatAgentLocation.EditingSession:
					...
					welcomeMessageContent = isToolsAgent ?
						{
							title: localize('editsTitle', "Edit with Assistant"),
							message: new MarkdownString(localize('agentMessage', "Ask the assistant to edit your files in agent mode. It will automatically use multiple requests to pick files to edit, run terminal commands, and iterate on errors.") + `\n\n${baseMessage}`),
							icon: Codicon.commentDiscussion
						} :
						{
							title: localize('editsTitle', "Edit with Assistant"),
							message: new MarkdownString(localize('editsMessage', "Start your editing session by defining a set of files that you want to work with. Then ask for the changes you want to make.") + `\n\n${baseMessage}`),
							icon: Codicon.commentDiscussion
						};
					break;
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpNeeded', "You need to set up the chat backend to use Chat."));

	private readonly _onUnresolvableError = this._register(new Emitter<void>());
	readonly onUnresolvableError = this._onUnresolvableError.event;

	private medgemmaClient = new MedGemmaClient();
	private summaryDetector = new MedicalSummaryDetector();
	// private workspaceFileScanner: WorkspaceFileScanner;
	...
	constructor(
		...
		super();
		// this.workspaceFileScanner = new WorkspaceFileScanner(fileService, workspaceContextService);
	}
	...
	async invoke(request: IChatAgentRequest, progress: (part: IChatProgress) => void, history: IChatAgentHistoryEntry[], token: CancellationToken): Promise<IChatAgentResult> {
		try {
			...
			const isSummaryRequest = this.summaryDetector.isSummaryRequest(request.message);

			if (isSummaryRequest) {
				this.logService.info('[MedGemma] Detected medical summary request');
				return await this.handleMedicalSummaryRequest(request, progress, token);
			}

			// Get tools from MCP service
			const tools = this.getToolsFromMCP();
			if (tools.length > 0) {
				this.logService.info(`[MedGemma] Passing ${tools.length} tools to backend`);
			} else {
				this.logService.info(`[MedGemma] No tools available from MCP`);
			}

			// Show processing message
			progress({
				kind: 'progressMessage',
				content: new MarkdownString('Processing with MedGemma...')
			});

			// Extract domain and mode from the widget
			const widget = this.instantiationService.invokeFunction(accessor => {
				const chatWidgetService = accessor.get(IChatWidgetService);
				return chatWidgetService.getWidgetBySessionId(request.sessionId);
			});

			const domain = widget?.input.currentDomain || 'general';
			const mode = widget?.input.currentMode || 'consult';

			this.logService.info(`[MedGemma] Request with domain=${domain}, mode=${mode}`);

			// Extract DICOM context from variables
			let imagePath: string | undefined;
			let enhancedMessage = request.message;

			// Look for DICOM implicit context in the variables
			const dicomVariable = request.variables.variables.find(v => v.id === 'vscode.implicit.dicom');
			if (dicomVariable && dicomVariable.value) {
				const dicomContext = dicomVariable.value as any;
				if (dicomContext.imagePath) {
					imagePath = dicomContext.imagePath;
					// Prepend metadata to message
					const metadataPrefix = `[DICOM Context]\nSlice: ${dicomContext.sliceIndex + 1}/${dicomContext.totalSlices}\nImage: ${dicomContext.imagePath}\n\n`;
					enhancedMessage = metadataPrefix + request.message;
					this.logService.info(`[MedGemma] Including DICOM context: slice ${dicomContext.sliceIndex + 1}/${dicomContext.totalSlices}`);
				}
			}

			// Look for file/text contexts (including PDF clipboard)
			const fileContexts = request.variables.variables.filter(v => {
				// Include if it has text value or is a file
				return (typeof v.value === 'string' && v.value.length > 0) ||
					v.isFile ||
					(v.name && v.name.includes('PDF'));
			});

			if (fileContexts.length > 0) {
				let contextPrefix = '[Additional Context]\n';
				for (const ctx of fileContexts) {
					// Get the value - it might be string or object
					let textContent = '';
					if (typeof ctx.value === 'string') {
						textContent = ctx.value;
					} else if (ctx.value && typeof ctx.value === 'object') {
						// Try to extract text from object
						textContent = (ctx.value as any).text || (ctx.value as any).content || JSON.stringify(ctx.value);
					}

					if (textContent.trim().length > 0) {
						contextPrefix += `\n--- ${ctx.name} ---\n${textContent}\n`;
						this.logService.info(`[MedGemma] Including context: ${ctx.name} (${textContent.length} chars)`);
					} else {
						this.logService.warn(`[MedGemma] Context ${ctx.name} has no text content. Value type: ${typeof ctx.value}`);
					}
				}
				contextPrefix += '\n[User Query]\n';
				enhancedMessage = contextPrefix + enhancedMessage;
			}

			// Create AbortController from CancellationToken
			...
			try {
				...
				// Stream response from MedGemma server
				await this.medgemmaClient.sendMessageStream(
					...
					tools // Pass tools from MCP
				);

				// After streaming completes, handle tool calls if any
				const toolHandler = new MedGemmaToolCallHandler(
					this.mcpService,
					this.medgemmaClient,
					this.logService
				);

				await toolHandler.handleResponse(
					request.sessionId,
					enhancedMessage,
					fullResponse,
					domain,
			...
			// Show error
			this.logService.error(`[MedGemma] Error: ${error}`);
			progress({
				kind: 'warning',
				content: new MarkdownString(`Error: ${(error as Error).message}`)
			});

			return { errorDetails: { message: (error as Error).message } };
		}
	}

	private getToolsFromMCP(): IToolSchema[] {
		try {
			// Log workspace information
			const workspace = this.workspaceContextService.getWorkspace();
			this.logService.info(`[MedGemma MCP] Workspace folders: ${workspace.folders.length}`);
			workspace.folders.forEach(folder => {
				this.logService.info(`[MedGemma MCP]   - ${folder.name}: ${folder.uri.toString()}`);
			});

			// Log MCP registry collections
			const registry = this.mcpService as any; // access internal registry
			if (registry._mcpRegistry) {
				const collections = registry._mcpRegistry.collections.get();
				this.logService.info(`[MedGemma MCP] MCP Registry has ${collections.length} collections`);
				collections.forEach((coll: any) => {
					const defs = coll.serverDefinitions.get();
					this.logService.info(`[MedGemma MCP]   Collection ${coll.id}: ${defs.length} server definitions`);
					defs.forEach((def: any) => {
						this.logService.info(`[MedGemma MCP]     - Definition: ${def.id} (${def.label})`);
					});
				});
			}
			...
			this.logService.info(`[MedGemma MCP] Found ${servers.length} MCP servers`);

			for (const server of servers) {
				this.logService.info(`[MedGemma MCP] Server: ${server.definition.id}, State: ${server.connectionState.get().state}`);

				// Get tools from this server
				const serverTools = server.tools.get();
				this.logService.info(`[MedGemma MCP] Server ${server.definition.id} has ${serverTools.length} tools`);

				for (const tool of serverTools) {
					// Convert MCP tool definition to our IToolSchema format
					const toolSchema = {
					...
					this.logService.info(`[MedGemma MCP] Added tool: ${tool.definition.name}`);
				}
			}

			this.logService.info(`[MedGemma MCP] Total tools collected: ${tools.length}`);
			return tools;
		} catch (error) {
			this.logService.error('[MedGemma MCP] Failed to fetch tools from MCP:', error);
			return [];
		}
	}

	private async handleMedicalSummaryRequest(request: IChatAgentRequest, progress: (part: IChatProgress) => void, token: CancellationToken): Promise<IChatAgentResult> {
		try {
			// Get workspace path
			...
			if (!workspacePath) {
				progress({
					kind: 'markdownContent',
					content: new MarkdownString('No workspace folder found. Please open a workspace folder to use the medical summary feature.')
				});
				return { metadata: { success: false, reason: 'no-workspace' } };
			}

			// Show scanning progress
			progress({
				kind: 'progressMessage',
				content: new MarkdownString('Scanning workspace for medical files...')
			});

			this.logService.info(`[MedGemma] Sending workspace path for summary: ${workspacePath}`);

			// Show generating progress
			progress({
				kind: 'progressMessage',
				content: new MarkdownString('Generating medical summary...')
			});
			...
			try {
				// Just send workspace path - backend does everything
				await this.medgemmaClient.sendMessageStream(
					request.sessionId,
					request.message,
					'general',
					...
					abortController.signal,
					undefined,  // no image
					workspacePath  // NEW: pass workspace path
				);

				return { metadata: { success: true, type: 'medical-summary' } };
			} finally {
				tokenListener.dispose();
			}

		} catch (error) {
			this.logService.error(`[MedGemma] Error generating medical summary: ${error}`);
			progress({
				kind: 'warning',
				content: new MarkdownString(`Error generating medical summary: ${(error as Error).message}`)
			});
	...
	// Commented out unused method - can be enabled if needed
	// private async _doInvoke(request: IChatAgentRequest, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService): Promise<IChatAgentResult> {
...
class ChatSetup {
	...
	// Commented out unused method - can be enabled if needed
	// @ts-ignore
	private async _showDialog(): Promise<ChatSetupStrategy> {
