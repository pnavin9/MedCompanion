...
export class ChatSubmitAction extends SubmitAction {
	...
	constructor() {
		const precondition = ContextKeyExpr.and(
			...
			ChatContextKeys.chatMode.isEqualTo(ChatMode.Consult),
		);
...
export const ToggleDomainActionId = 'workbench.action.chat.toggleDomain';

export interface IToggleChatModeArgs {
	mode: ChatMode;
}

export interface IToggleDomainArgs {
	domain: ChatDomain;
}
...
class ToggleChatModeAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: any[]) {
		...
		const needToClearEdits = (!chatService.unifiedViewEnabled || (!configurationService.getValue(ChatConfiguration.Edits2Enabled) && (context.chatWidget.input.currentMode === ChatMode.Plan || switchToMode === ChatMode.Plan))) && requestCount > 0;
	...
	private getNextMode(chatWidget: IChatWidget, requestCount: number, configurationService: IConfigurationService): ChatMode {
		...
		if (configurationService.getValue(ChatConfiguration.Edits2Enabled) || requestCount === 0) {
			modes.push(ChatMode.Plan);
			modes.push(ChatMode.Diagnose);
		}
		if (chatWidget.location === ChatAgentLocation.Panel) {
			modes.push(ChatMode.Consult);
		}
...
class ToggleDomainAction extends Action2 {
	static readonly ID = ToggleDomainActionId;

	constructor() {
		super({
			id: ToggleDomainAction.ID,
			title: localize2('interactive.toggleDomain.label', "Set Medical Domain"),
			f1: true,
			category: CHAT_CATEGORY,
			precondition: ChatContextKeys.enabled,
			tooltip: localize('setDomain', "Set Domain"),
			menu: [
				{
					id: MenuId.ChatExecute,
					order: 2,
	...
	async run(accessor: ServicesAccessor, ...args: any[]) {
		...
		const requestArgs: IToggleDomainArgs = args[0];
		const domain = validateChatDomain(requestArgs?.domain);
		if (domain) {
			widget.input.setChatDomain(domain);
		}
	}
}
...
export class ChatEditingSessionSubmitAction extends SubmitAction {
	...
	constructor() {
		const precondition = ContextKeyExpr.and(
			...
			ContextKeyExpr.or(ChatContextKeys.inputHasText, ChatContextKeys.instructionsAttached),
			whenNotInProgressOrPaused,
			ChatContextKeys.chatMode.notEqualsTo(ChatMode.Consult),
		);
			...
			menu: [
				{
					id: MenuId.ChatExecuteSecondary,
					group: 'group_1',
					when: ContextKeyExpr.and(whenNotInProgressOrPaused, ChatContextKeys.chatMode.notEqualsTo(ChatMode.Agent),),
					order: 1
				},
				{
					id: MenuId.ChatExecute,
...
export function registerChatExecuteActions() {
	...
	registerAction2(ToggleDomainAction);
