...
export function registerChatTitleActions() {
	...
	registerAction2(class RetryChatAction extends Action2 {
		...
		async run(accessor: ServicesAccessor, ...args: any[]) {
			...
			if (chatModel?.initialLocation === ChatAgentLocation.EditingSession || chatModel && (mode === ChatMode.Plan || mode === ChatMode.Agent || mode === ChatMode.Diagnose)) {
	...
	registerAction2(class RemoveAction extends Action2 {
		constructor() {
			super({
				...
				category: CHAT_CATEGORY,
				icon: Codicon.x,
				precondition: ContextKeyExpr.and(ChatContextKeys.chatMode.isEqualTo(ChatMode.Consult), ChatContextKeyExprs.unifiedChatEnabled.negate()),
				keybinding: {
					primary: KeyCode.Delete,
					mac: {
						primary: KeyMod.CtrlCmd | KeyCode.Backspace,
					},
					when: ContextKeyExpr.and(ChatContextKeys.chatMode.isEqualTo(ChatMode.Consult), ChatContextKeys.inChatSession, ChatContextKeys.inChatInput.negate()),
				...
				menu: {
					...
					group: 'navigation',
					order: 2,
					when: ContextKeyExpr.and(ChatContextKeys.chatMode.isEqualTo(ChatMode.Consult), ChatContextKeys.isRequest, ChatContextKeyExprs.unifiedChatEnabled.negate())
				}
			});
		}
