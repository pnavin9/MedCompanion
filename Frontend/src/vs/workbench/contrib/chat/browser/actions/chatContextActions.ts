...
class AddPdfClipboardToChatAction extends Action2 {
	static readonly ID = 'workbench.action.chat.addPdfClipboard';

	constructor() {
		super({
			id: AddPdfClipboardToChatAction.ID,
			title: localize2('workbench.action.chat.addPdfClipboard.label', "Add PDF Clipboard to Chat"),
			...
			keybinding: {
				...
				when: ContextKeyExpr.and(
					...
					ContextKeyExpr.has('editorLangId').negate() // Only when not in a text editor
				)
			}
		});
	}
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		try {
			...
			if (trimmed.length < 20) {
				notificationService.warn(localize('pdfClipboard.tooShort', "Clipboard text is too short (minimum 20 characters)"));
				return;
			}

			if (trimmed.length > 10000) {
				notificationService.warn(localize('pdfClipboard.tooLong', "Clipboard text is too long (maximum 10,000 characters)"));
				return;
			}

			// Check if current file is a PDF (optional - add context regardless)
			const activeEditor = editorService.activeEditor;
			const pdfUri = activeEditor?.resource;
			const isPdf = pdfUri && pdfUri.path.toLowerCase().endsWith('.pdf');

			// Show chat view and add context
			const chatView = await showChatView(viewsService);
			if (chatView) {
				// Create a virtual "file" entry for the PDF excerpt
				const contextName = isPdf && pdfUri
					? `PDF: ${basename(pdfUri)}`
					: 'PDF Clipboard';

				// Add the text directly to the attachment model (not via variablesService which treats it as file)
				chatView.attachmentModel.addContext({
					id: `pdf-clipboard-${Date.now()}`,
					name: contextName,
					value: trimmed,
					isFile: false
				});
				...
				chatView.focusInput();
				notificationService.info(localize('pdfClipboard.added', "Added clipboard text to chat context ({0} characters)", trimmed.length));
			}
		} catch (error) {
			notificationService.error(localize('pdfClipboard.error', "Failed to add clipboard to chat: {0}", String(error)));
		}
	}
}
...
export function registerChatContextActions() {
	...
	registerAction2(AddPdfClipboardToChatAction);
}
