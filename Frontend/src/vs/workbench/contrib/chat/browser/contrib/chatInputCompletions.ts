	...
	constructor(
		...
		this._register(this.languageFeaturesService.completionProvider.register({ scheme: ChatInputPart.INPUT_SCHEME, hasAccessToAllModels: true }, {
			...
			provideCompletionItems: async (model: ITextModel, position: Position, _context: CompletionContext, token: CancellationToken) => {
				...
				if (!widget || !viewModel || widget.input.currentMode !== ChatMode.Consult) {
					return;
				}
		...
		this._register(this.languageFeaturesService.completionProvider.register({ scheme: ChatInputPart.INPUT_SCHEME, hasAccessToAllModels: true }, {
			...
			provideCompletionItems: async (model: ITextModel, position: Position, _context: CompletionContext, token: CancellationToken) => {
				...
				const widget = this.chatWidgetService.getWidgetByInputUri(model.uri);
				if (widget?.location !== ChatAgentLocation.Panel || widget.input.currentMode !== ChatMode.Consult) {
					return;
				}
...
class BuiltinDynamicCompletions extends Disposable {
	...
	private async addFileEntries(widget: IChatWidget, result: CompletionList, info: { insert: Range; replace: Range; varWord: IWordAtPosition | null }, token: CancellationToken) {
		...
		// RELATED FILES
		if (widget.input.currentMode !== ChatMode.Consult && widget.viewModel && widget.viewModel.model.editingSession) {
