...
import { HasSpeechProvider, ISpeechService, ISpeechToTextSession, KeywordRecognitionStatus, SpeechToTextInProgress, SpeechToTextStatus, TextToSpeechStatus, TextToSpeechInProgress as GlobalTextToSpeechInProgress } from '../../../speech/common/speechService.js';
...
interface IActiveVoiceChatSession extends IVoiceChatSession {
	...
	voiceChatSession?: ISpeechToTextSession; // Store reference to call complete() before disposal
}
...
class VoiceChatSessions {
	...
	async start(controller: IVoiceChatSessionController, context?: IChatExecuteActionContext): Promise<IVoiceChatSession> {
		...
		// Store reference to the session so we can call complete() before disposal
		session.voiceChatSession = voiceChatSession;
		...
		// Track cleanup state to prevent double-disposal
		let cleanedUp = false;
		const cleanup = () => {
			if (cleanedUp) {
				return;
			...
			// Cancel the token to trigger service-level cleanup before disposing CTS
		...
		// DON'T add onDidChange listener to session.disposables!
		// It needs to stay alive to receive async Recognized events after stop() is called.
		const onDidChangeDisposable = voiceChatSession.onDidChange(({ status, text, waitingForInput }) => {
			// Allow Recognized and Stopped events through even when token is cancelled
			if (cts.token.isCancellationRequested && status !== SpeechToTextStatus.Recognized && status !== SpeechToTextStatus.Stopped) {
				return;
			}
			...
			switch (status) {
				...
				case SpeechToTextStatus.Recognized:
					...
					// Clean up after async event processing completes
					setTimeout(cleanup, 100);
					break;
				case SpeechToTextStatus.Stopped:
					// Set timeout to clean up if Recognized never arrives
					setTimeout(() => {
						if (!cleanedUp) {
							cleanup();
						}
		...
		// Cleanup on component destruction (external token cancellation)
		const tokenDisposable = cts.token.onCancellationRequested(() => {
			cleanup();
		});

		session.disposables.add(tokenDisposable);
	...
	stop(voiceChatSessionId = this.voiceChatSessionIds, context?: VoiceChatSessionContext): void {
		...
		// Complete the session before disposal to fire final events
		if (this.currentVoiceChatSession.voiceChatSession && 'complete' in this.currentVoiceChatSession.voiceChatSession) {
			(this.currentVoiceChatSession.voiceChatSession as any).complete();
		}
