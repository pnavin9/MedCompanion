...
```mermaid
graph TB
    subgraph "VS Code Side"
        A1[mcp.json Config] --> A2[Config Discovery]
        A2 --> A3[MCP Registry]
        A3 --> A4[MCP Service]
        A4 --> A5[MCP Server Connection]
        A5 --> A6[Python Process]
        A7[MedGemma Chat Agent] --> A8[MedGemma Client]
        A7 --> A9[Tool Call Handler]
        A9 --> A4
        A7 --> A4
    end
    
    subgraph "Backend Side"
        B1[FastAPI /api/v1/chat/stream] --> B2[MedGemma Service]
        B2 --> B3[LLM Model]
    end
    
    subgraph "MCP Server Process"
        C1[arithmetic_server.py] --> C2[Tool Implementations]
    end
    
    A8 <-->|HTTP POST / SSE| B1
    A5 <-->|JSON-RPC over stdio| C1
    A4 -->|Tool Schemas| A8
    A8 -->|Tool Schemas| B1
    B1 -->|LLM Response| A8
    A9 -->|Tool Execution| A4
    A4 -->|Tool Results| A9
    A9 -->|Tool Results| A8
    A8 -->|Tool Results| B1
